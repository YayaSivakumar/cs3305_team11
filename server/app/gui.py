...
class UploadThread(QThread):
    ...
    def __init__(self, url, filePath, title, message, persistence_hours, password):
        super().__init__()
        self.url = url
    ...
    def run(self):
        try:
            with open(self.filePath, 'rb') as f:
                ...
                data = {'message': self.message, 'title': self.title, 'expiration_hours': self.persistence_hours, 'password': self.password}
...
class FileUploader(QWidget):
    ...
    def initUI(self):
        ...
        self.setGeometry(100, 100, 400, 350)  # Adjusted for additional button
        ...
        self.persistenceComboBox.addItems(['1-Day', '3-Days', '7-Days'])
    ...
    def uploadAndShare(self):
        if self.filePath:
            ...
            self.uploadThread = UploadThread(url, self.filePath, message, persistence_hours, password)
    ...
    def onUploadFinished(self, response_text):
        ...
        # Example of setting the expiration label, assuming you can extract or calculate the expires_at value
        expires_at = "24 hours"  # This should be dynamically determined based on your application's logic
        self.expiresInLabel.setText(f'Expires in: {expires_at}')
        self.expiresInLabel.show()  # Make sure to show the label now that it has relevant information
